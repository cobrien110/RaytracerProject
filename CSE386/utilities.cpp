...
double distanceFromOrigin(double x, double y) {
	/* CSE 386 - todo  */
	return glm::distance(dvec2(x, y), dvec2(0, 0));
...
double areaOfTriangle(double a, double b, double c) {
	...
	double s = (a + b + c) / 2.0;
	double area = glm::sqrt(s * (s - a) * (s - b) * (s - c));
	if (glm::isnan(area) || glm::isinf(area)) {
		return -1;
	}
	return area;
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
	/* CSE 386 - todo  */
	double x = center.x + cos(angleRads * R);
	double y = center.y + sin(angleRads * R);
	return dvec2(x, y);
}
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	/* CSE 386 - todo  */
	dvec2 translatePt = targetPt - referencePt;
	return normalizeRadians(glm::atan(translatePt.y, translatePt.x));
}
...
double map(double x, double fromLo, double fromHi, double toLow, double toHigh) {
	/* CSE 386 - todo  */
	// NOTE that if fromHi and fromLo are the same value, the result is -inf.
...
vector<double> quadratic(double A, double B, double C) {
	/* CSE 386 - todo  */
	double discriminate = glm::pow(B, 2) - 4 * A * C;
	vector<double> result;	// put only the roots in here

	if (discriminate > 0) { // 2 roots
		double rootA = (-B + glm::sqrt(discriminate)) / (2 * A);
		double rootB = (-B - glm::sqrt(discriminate)) / (2 * A);
		// Place in vector in ascending order
		result.push_back(glm::min(rootA, rootB));
		result.push_back(glm::max(rootA, rootB));
	}
	else if (discriminate == 0) { // 1 root
		result.push_back(-B / (2 * A));
		// discriminate is zero so no need to include it in calculation
	}
	// Else, no roots, return empty result
	return result;
}
...
int quadratic(double A, double B, double C, double roots[2]) {
	/* CSE 386 - todo  */
	vector <double> foundRoots = quadratic(A, B, C);
	int rootCnt = foundRoots.size();
	if (rootCnt == 2) {
...
dvec3 myNormalize(const dvec3& V) {
	...
	return dvec3(V.x / vLen, V.y / vLen, V.z / vLen);
}
...
bool formAcuteAngle(const dvec3& a, const dvec3& b) {
	...
	double vLenA = glm::length(a);
	double vLenB = glm::length(b);

	double cos = dot / (vLenA * vLenB);

	// cos will be positive if the calculated value is greater than zero
	return (cosBetween(a,b) > 0.0);
}
...
double cosBetween(const dvec2& v1, const dvec2& v2) {
	// (a dot b) / (length of a * length of b) = cos(alpha)
	double dot = glm::dot(v1, v2);
	double vLenA = glm::length(v1);
	double vLenB = glm::length(v2);
...
vector<dvec3> multiplyMatrixAndVertices(const dmat3& M, const vector<dvec3>& verts) {
	/* CSE 386 - todo  */
	vector<dvec3> result = verts;
	for (int i = 0; i < verts.size(); i++) {
		result[i] = multiplyMatrixAndVertex(M, verts[i]);
	}
	return result;
}
...
vector<dvec3> multiplyMatricesAndVertices(const vector<dmat3>& M, const vector<dvec3>& verts) {
	...
	vector<dvec3> result = verts;
	for (int i = 0; i < verts.size(); i++) {
		result[i] = multiplyMatrixAndVertex(multiplyMatrices(M), verts[i]);
	}
	return result;
}
