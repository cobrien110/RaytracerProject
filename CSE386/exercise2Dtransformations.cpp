...
void doScale2XAboutCenterOfTriangle() {
	dmat3 t1 = glm::inverse(T(-1.5 * N, 2.5 * N));
	dmat3 s = S(3, 3);
	dmat3 t2 = glm::inverse(t1);
...
void doReflectAcrossLineYeqXplus50() {
	dmat3 t1 = T(50, 0);
	dmat3 r1 = R(deg2rad(45));
	dmat3 s = S(-1, 1);
	dmat3 r2 = glm::inverse(r1);
	dmat3 t2 = glm::inverse(t1);


	dmat3 TM = t2 * r2 * s * r1 * t1;
...
void doSquareRotatingAroundOwnAxisAndAroundSun() {
	...
	dmat3 r1 = R(angleInRads * 4);
	dmat3 t = T(distance, 0);
	dmat3 r2 = R(angleInRads * 0.5);
...
vector<DisplayFunc> funcs = {
								...
								DisplayFunc(doScale2XAboutCenterOfTriangle, "6. Scale 3X about tri center point"),
